/*
  Official SoftDevArts lockup from public/logo-softdevarts.svg, inlined so it
  can adapt to the surface: the bracket mark stays brand-gold (#EBA900), while
  the "SOFTDEVARTS" wordmark uses currentColor (near-black on light surfaces,
  off-white on the dark footer via `tone`). viewBox is cropped to the artwork.
*/
export default function Logo({
  tone = "dark",
  size = "h-11",
  className = "",
}: {
  tone?: "dark" | "light";
  size?: string;
  className?: string;
}) {
  const text = tone === "light" ? "text-base" : "text-fg";
  return (
    <svg
      viewBox="28 116 540 360"
      role="img"
      aria-label="SoftDevArts"
      className={`${size} w-auto ${text} ${className}`}
      fill="currentColor"
    >
      {/* Wordmark (inherits currentColor) */}
      <g>
        <path d="M263,157.1c-0.7-4.5-4.3-12-16-12c-8,0-13.1,5-13.1,10.3c0,4.6,2.8,8,9.5,9.2l12.8,2.4c18.2,3.4,27.7,15.1,27.7,28.8c0,15-12.6,30.3-35.4,30.3c-26.5,0-37.9-17.2-39.2-30.7l19.6-4.7c0.8,8.8,6.8,16.5,19.9,16.5c8.4,0,13.5-4.1,13.5-10c0-4.9-3.9-8.4-10.3-9.6l-13.1-2.4c-16.2-3.1-26.1-13.9-26.1-28.1c0-18.1,16-30.8,34.3-30.8c23.9,0,33,14.3,35,25.5L263,157.1z" />
        <path d="M342.7,126.1c26.9,0,49.7,19.6,49.7,50c0,30.3-22.8,49.9-49.7,49.9c-27,0-49.9-19.6-49.9-49.9C292.8,145.7,315.7,126.1,342.7,126.1z M342.7,204.9c13.2,0,27.6-8.9,27.6-28.9s-14.3-29.1-27.6-29.1c-13.4,0-27.7,9.1-27.7,29.1S329.3,204.9,342.7,204.9z" />
        <path d="M407.6,226.1v-100H469v20.1h-40.1v20.1h35.3v19.3H429v40.4H407.6z" />
        <path d="M531.8,146.4v79.7h-21.2v-79.7h-29.3v-20.3h79.9v20.3H531.8z" />
        <path d="M289.2,346.5v-99.9h36.2c30,0,50.1,19.2,50.1,50c0,30.9-20.1,49.9-50.3,49.9H289.2z M324.5,325.5c15.4,0,27.9-8.9,27.9-28.9c0-20-12.5-29-27.9-29h-13v57.9H324.5z" />
        <path d="M391.5,346.5v-99.9h63.3v21h-41v19H451v19.6h-37.2v19.2h41.1v21.1H391.5z" />
        <path d="M537.3,246.6H561l-36.5,99.9h-23l-36.6-99.9h24.5l24.1,70.3L537.3,246.6z" />
        <path d="M446.6,387.3V467h-21.2v-79.7h-29.3V367H476v20.3H446.6z" />
        <path d="M229.4,466.9h-23.7l36.5-99.9h23l36.6,99.9h-24.5l-24.1-70.3L229.4,466.9z" />
        <path d="M345,429.8h-7.8v36.9h-22.1v-99h39.2c19.5,0,32.1,13.4,32.1,31c0,13.4-7.4,23.6-19.5,27.9l20.1,40.1h-24.3L345,429.8z M350.2,410.8c9.1,0,14-5,14-12c0-7.4-4.9-12-14-12h-13v24H350.2z" />
        <path d="M539.5,397.6c-0.7-4.6-4.5-12.4-16.5-12.4c-8.2,0-13.5,5.2-13.5,10.6c0,4.7,2.9,8.2,9.8,9.5l13.3,2.5c18.9,3.5,28.6,15.6,28.6,29.7c0,15.5-13,31.3-36.6,31.3c-27.4,0-39.1-17.7-40.5-31.7l20.2-4.9c0.8,9.1,7,17,20.5,17c8.7,0,14-4.2,14-10.3c0-5-4-8.7-10.6-9.9l-13.5-2.5c-16.8-3.2-27-14.4-27-29c0-18.7,16.5-31.8,35.5-31.8c24.7,0,34.1,14.8,36.2,26.4L539.5,397.6z" />
      </g>
      {/* Bracket mark (always brand-gold) */}
      <g fill="#EBA900">
        <path d="M98.9,235.5c14.9,0,30.4-7,30.4-26v-35.3c0-31.9,16.2-47.9,46.4-47.9h10.9v19.3h-10.9c-19.1,0-25.2,8.2-25.2,27v42.2c0,17.3-10.3,28.6-26.8,31.7c16.5,3.1,26.8,14.4,26.8,31.7v42c0,18.8,6.2,27.1,25.2,27.1h11.1v19.3h-11.1c-30.1,0-46.4-16-46.4-47.9v-35c0-19.1-15.4-26-30.4-26V235.5z" />
        <path d="M34,235.5c14.9,0,30.4-7,30.4-26v-35.3c0-31.9,16.2-47.9,46.4-47.9h10.4v19.3h-10.4c-19.1,0-25.2,8.2-25.2,27v42.2c0,17.3-10.3,28.6-26.8,31.7c16.5,3.1,26.8,14.4,26.8,31.7v42c0,18.8,6.2,27.1,25.2,27.1h10.4v19.3h-10.4c-30.1,0-46.4-16-46.4-47.9v-35c0-19.1-15.4-26-30.4-26V235.5z" />
      </g>
    </svg>
  );
}
